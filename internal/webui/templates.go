        ...
        document.getElementById('walkBtn').addEventListener('click', async function(){
            ...
            try{
                ...
                (state.walk.interfaces || []).forEach(function(it){
                    // Explicit existing config wins; fall back to rule defaults.
                    var ex = it.existing_config;
                    var ruleMonitor = (it.rule_monitor != null) ? !!it.rule_monitor : false;
                    var defMon = ex ? !!ex.monitor : ruleMonitor;
                    var defAlias = (ex && ex.description) ? ex.description : (it.alias || '');
                    var defDesired = (ex && ex.desired_state) ? ex.desired_state : (it.rule_desired_state || (it.oper_status === 'up' ? 'up' : 'down'));
                    var defSev = (ex && ex.alert_severity) ? ex.alert_severity : (it.rule_severity || 'warning');
